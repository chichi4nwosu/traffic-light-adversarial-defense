...
def attack_effectiveness(params, patch, device, model, loader, name, epoch):
    ...
    for batch in tqdm(loader, desc="Evaluating", leave=False):
        ...
        x_center   = lowest_bbox[0].item() * params["img_size"]
        y_center   = lowest_bbox[1].item() * params["img_size"]
        bbox_width  = lowest_bbox[2].item() * params["img_size"]
        bbox_height = lowest_bbox[3].item() * params["img_size"]
        ...
        if (patch_end_x > params["img_size"] or patch_end_y > params["img_size"]
